...
import { faArrowsUpDown, faArrowUpLong, faArrowDownLong } from '@fortawesome/free-solid-svg-icons';
...
function CustomTR(_ref) {
  var item = _ref.item,
      columns = _ref.columns;
  var customTD = columns.map(function (column, index) {
    return /*#__PURE__*/React.createElement("td", {
      key: index,
      className: "p-3 border border-neutral-800 border-collapse text-center"
    }, item[column.key]);
  });
  return /*#__PURE__*/React.createElement("tr", {
    className: "odd:bg-neutral-600 even:bg-neutral-700"
  }, customTD);
}
CustomTR.propTypes = {
  item: propTypes.exports.any.isRequired,
  columns: propTypes.exports.array.isRequired
};

// Returns a function responsible for sorting a specific column index 
// (key = columnKey, asc = ascending order?).
var Comparer = function Comparer(key, asc) {
  ...
  return function (item1, item2) {
    ...
    return function (value1, value2) {
      ...
      return value1 !== "" && value2 !== "" && !isNaN(value1) && !isNaN(value2) ? value1 - value2 : value1.toString().localeCompare(value2);
    }(asc ? item1[key] : item2[key], asc ? item2[key] : item1[key]);
  };
};

// Constants
var PAGE_LIMIT = 4; // Returns an array of page indexes to display with buttons or spans 
// calculated from the currentPage index and the pageCount (max number of pages)

var Paginator = function Paginator(currentPage, pageCount) {
  var pages = []; // If there is 6 or less pages, we show each of them
  ...
  if (currentPage <= PAGE_LIMIT) {
    for (var _i = 1; _i <= PAGE_LIMIT + 1; _i++) {
      pages.push(_i);
    }
  ...
  if (pageCount - currentPage < PAGE_LIMIT) {
    for (var _i2 = pageCount; _i2 >= pageCount - PAGE_LIMIT; _i2--) {
      pages.push(_i2);
    }
...
function ReactDataTable(_ref) {
  ...
  useEffect(function () {
    // Creating the data
    var initData = data.data.map(function (item) {
      return _objectSpread2(_objectSpread2({}, item), {}, {
        "show": true
      });
    });
    sortedData = initData; // Creating the columns

    var initColumns = data.columns.map(function (column) {
    ...
    columns = initColumns;
  }, [data]); // Sorting table data by value of column clicked
  // @param e: event of the click
  // @param column: column clicked

  var sortByColumn = function sortByColumn(e, column) {
    e.preventDefault();
    e.stopPropagation(); // Switching icons and sorting method for the column clicked
    ...
    var newlySortedData = _toConsumableArray(sortedData); // Sorting the data by result to the Comparer function call
  ...
  var handleSearch = function handleSearch(e) {
    var searchString = e.target.value; // If no input string is specified, we show everything
    ...
    if (searchString === "") {
      ...
      sortedData.forEach(function (item) {
        item.show = columns.some(function (column) {
          return item[column.key].toLowerCase().includes(searchString.toLowerCase());
        });
      });
      setIsSearched(true);
    }

    var newlySortedData = _toConsumableArray(sortedData);
  ...
  var nbEntries = sortedData.filter(function (item) {
    return item.show;
  }).length; // Rendering the header cells

  var customTH = columns.map(function (item, index) {
    return /*#__PURE__*/React.createElement("th", {
      key: index,
      className: "p-3 border border-neutral-800 border-collapse cursor-pointer",
      onClick: function onClick(e) {
        return sortByColumn(e, item);
      }
  ...
  var pageCount = Math.ceil(nbEntries / maxRows); // Handling the click on the Previous button
  ...
  var handleGoToPage = function handleGoToPage(value) {
    setCurrentPage(value);
  };

  var pageButtons = [];
  var previousButtonDisabled, nextButtonDisabled; // Rendering the pages buttons

  if (pageCount > 0) {
    // Getting the pages to display
    var pages = Paginator(currentPage, pageCount); // If there is at least one page to show, the disabled attribute of the Previous and Next button is calculated from the current page index

    previousButtonDisabled = currentPage === 1;
    nextButtonDisabled = currentPage === pageCount;
    pageButtons = pages.map(function (item, index) {
      if (item === currentPage) {
  ...
  var nbRows = 0; // Rendering the rows according to the max rows to show on a page and the current page index

  var rows = sortedData.map(function (item, index) {
    if (item.show && nbRows < maxRows && index + 1 > (currentPage - 1) * maxRows) {
      nbRows++;
      return /*#__PURE__*/React.createElement(CustomTR, {
        key: index,
        item: item,
  ...
  var entriesSummary = isSearched ? /*#__PURE__*/React.createElement("span", {
    className: "text-white"
  }, "Showing ", startRowCount, " to ", endRowCount, " of ", nbEntries, " entries (filtered from ", sortedData.length, " total entries)") : /*#__PURE__*/React.createElement("span", {
    className: "text-white"
  }, "Showing ", startRowCount, " to ", endRowCount, " of ", nbEntries, " entries");
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    ...
    className: "text-white"
  }, "entries")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", {
    ...
    className: "border border-neutral-800 text-white"
  }, /*#__PURE__*/React.createElement("thead", null, /*#__PURE__*/React.createElement("tr", null, customTH)), /*#__PURE__*/React.createElement("tbody", null, rows))), /*#__PURE__*/React.createElement("div", {
    className: "flex justify-between items-center mt-3"
  }, /*#__PURE__*/React.createElement("div", null, entriesSummary), /*#__PURE__*/React.createElement("div", {
  ...
  }, "Previous"), /*#__PURE__*/React.createElement("div", null, pageButtons), /*#__PURE__*/React.createElement("button", {
    id: "nextButton",
...
ReactDataTable.propTypes = {
  data: propTypes.exports.object.isRequired
};
